...
All development is to follow the forked [git-flow](http://nvie.com/posts/a-successful-git-branching-model/) process, modified to allow for code-reviews.
...
You can install `git-flow` via either

```sh
brew install git-flow
```

or you can use the AVH edition of Git Flow which adds some handy features. In which case use these [`git-flow-completion` utils](https://github.com/petervanderdoes/git-flow-completion) instead.
...
Make sure `git` knows your correct email.
...
Note `git-flow` will create a branch called `feature/{issue number}/{some_descriptive-text}`.
...
When merging a pull request into `develop` you might want to consider doing a 'squash merge' if your PR contains a lot of small commits.  In general however we believe it's better to have more detail in the commit log, so 'squash merges' should be the exception rather than the rule.  Use your common sense here.

Merges to `master` must never be squashed.
